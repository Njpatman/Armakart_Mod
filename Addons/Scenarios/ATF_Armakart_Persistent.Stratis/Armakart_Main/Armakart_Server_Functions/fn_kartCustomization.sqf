...
["Edit_kartCustomization"] remoteExec ["NJP_Client_Fnc_GUI_Edit", 0, false];
...
kartCustomization_Time = 35;

[_color, kartCustomization_Time] remoteExec ["NJP_Client_fnc_Update_Timer", 0, true];

uiSleep 1;

_Customization_time_Left = kartCustomization_Time + time;
...
while {_Time_left > 0} do {
	...
	if (_Time_left < (kartCustomization_Time * 0.35)) then {_color = "#eef441";};//yellow

	if (_Time_left < (kartCustomization_Time * 0.15)) then {_color = "#ff0000";};//red
